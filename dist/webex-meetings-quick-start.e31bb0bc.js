...
var webex; // First, let's wire our form fields up to localStorage so we don't have to
// retype things everytime we reload the page.

['access-token', 'invitee'].forEach(function (id) {
  ...
  return new Promise(function (resolve) {
    if (!webex) {
      ...
      webex = window.webex = Webex.init({
        config: {
          meetings: {
            deviceType: 'WEB' // Any other sdk config we need
    ...
    webex.meetings.on('meeting:added', function (addedMeetingEvent) {
      if (addedMeetingEvent.type === 'INCOMING') {
        var addedMeeting = addedMeetingEvent.meeting; // Acknowledge to the server that we received the call on our device

        addedMeeting.acknowledge(addedMeetingEvent.type).then(function () {
    ...
    if (!webex.meetings.registered) {
      ...
      .then(function () {
        return webex.meetings.syncMeetings();
      }).then(function () {
        // This is just a little helper for our selenium tests and doesn't
        // really matter for the example
        document.body.classList.add('listening');
        document.getElementById('connection-status').innerHTML = 'connected'; // Our device is now connected
...
function bindMeetingEvents(meeting) {
  ...
  // handler.
  meeting.on('error', function (err) {
    console.error(err);
  }); // Handle media streams changes to ready state

  meeting.on('media:ready', function (media) {
    if (!media) {
      return;
    }
  ...
  meeting.on('media:stopped', function (media) {
  ...
  meeting.members.on('members:update', function (delta) {
    var membersData = delta.full;
    var memberIDs = Object.keys(membersData);
    memberIDs.forEach(function (memberID) {
      var memberObject = membersData[memberID]; // Devices are listed in the memberships object.
...
function joinMeeting(meeting) {
  ...
  return meeting.join().then(function () {
    return meeting.getSupportedDevices({
      ...
      return meeting.getMediaStreams(mediaSettings).then(function (mediaStreams) {
        var _mediaStreams = _slicedToArray(mediaStreams, 2),
            localStream = _mediaStreams[0],
            localShare = _mediaStreams[1];
...
document.getElementById('dialer').addEventListener('submit', function (event) {
  // again, we don't want to reload when we try to dial
  event.preventDefault();
  var destination = document.getElementById('invitee').value; // we'll use `connect()` (even though we might already be connected or
  // connecting) to make sure we've got a functional webex instance.

  connect().then(function () {
    // Create the meeting
    return webex.meetings.create(destination).then(function (meeting) {
...
window.addEventListener('load', function () {
  ...
  var audio = document.getElementById('constraints-audio');
  var video = document.getElementById('constraints-video'); // Get access to hardware source of media data
  ...
  if (navigator && navigator.mediaDevices && navigator.mediaDevices.enumerateDevices) {
    navigator.mediaDevices.enumerateDevices().then(function (devices) {
      ...
      var hasAudio = devices.filter(function (device) {
        return device.kind === 'audioinput';
      }).length > 0; // Check/uncheck and disable checkbox (if necessary) based on the results from the API

      audio.checked = hasAudio;
      audio.disabled = !hasAudio; // Check if navigator has video

      var hasVideo = devices.filter(function (device) {
...
//# sourceMappingURL=/webex-meetings-quick-start.e31bb0bc.js.map